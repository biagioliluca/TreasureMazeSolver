...
class TreasureMazeProblem(Problem):
  ...
  def update_grid(self, actions):
    '''
      Update problem's grid modifying S, X and T cells involve on the found path
    '''
...
def calculate_heuristic_grid(treasure_maze_problem):
  '''
    Calculates a grid of heuristic value used on A* algorithm 
  '''
  treasure_positions = []
  heuristic_grid = []

  # find treasures' coords
  ...
  # check if there is no treasure in the grid
  if len(treasure_positions) == 0:
    ...
    for i in range(len(treasure_maze_problem.grid)):
      row = []
      for j in range(len(treasure_maze_problem.grid[i])):
        # for each elements of grid calculates all the h values associated to each treausre and choose the minor
        h_values = []
...
def is_in(node, queue):
  '''
    Function that checks if a node is in the passed queue (referring to state values)
  '''
  if len(queue.heap) == 0:
    return False
  return any(node.state[0] == x.state[0] and node.state[1] == x.state[1] for _ , x in queue.heap)

def dijkstra(problem, f):
  '''
    Djikstra algorithm
  '''
